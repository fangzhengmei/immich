...
**重要说明**：带密码分享链接的 `SharedLinkToken` Cookie 校验**仅发生在 `/shared-links/me` 接口链路**，其他标注 `sharedLink: true` 的路由不校验密码 Token。

#### 鉴权范围对照表：

| 接口路由 | 鉴权机制 | 是否校验Cookie Token |
|---------|---------|-------------------|
| `GET /shared-links/me` | ① shareKey 认证层校验 → ② Cookie Token 密码校验 | ✅ 是 |
| `POST /shared-links/login` | ① shareKey 认证层校验 → ② 请求体 password 校验 | ❌ 否（首次登录） |
| `GET /shared-links/:id` | 仅 Session/API Key 权限校验 | ❌ 否 |
| 其他 `sharedLink: true` 路由（资产下载、缩略图等） | 仅 shareKey/slug + 认证层过期校验 | ❌ 否 |

#### 密码登录流程（`POST /shared-links/login`）：

```typescript
// shared-link.service.ts:login
...
```typescript
@Authenticated({ sharedLink: true })  // 允许共享链接访问此路由（仅shareKey+过期校验，不校验密码）
...
checkAccess() 【统一权限入口】
    ...
    │   ├─ 公开链接：按链接配置裁剪
    │   └─ 密码链接：仅 /me 校验Cookie Token，其余路由同公开链接
...
| 处理阶段 | 同事相册共享（普通用户路径） | 带密码分享链接（共享链接路径） | 公开分享链接（共享链接路径） |
|---------|---------------------------|---------------------------|--------------------------|
| **【认证层处理】** | | | |
| 认证方式 | Session / API Key | shareKey/slug 密钥认证 | shareKey/slug 密钥认证 |
| 过期校验 | 不适用 | 认证层校验 `expiresAt` | 认证层校验 `expiresAt` |
| 密码校验 | 不适用 | 仅 `/shared-links/me` 和 `/login` 校验 | 无密码 |
| **【权限裁剪层处理】** | | | |
| 统一入口 | `checkOtherAccess()` | `checkSharedLinkAccess()` | `checkSharedLinkAccess()` |
| AssetRead 裁剪 | 所有者资产 ∪ 加入相册资产 ∪ 伴侣共享资产 | 共享链接关联资产 | 共享链接关联资产 |
...
公开链接和密码链接共用此控制逻辑，**密码链接仅在 `/shared-links/me` 接口额外验证 Cookie Token**：
...
- 密码保护链接需同时持有 **链接密钥** + **访问密码** 双重认证（仅 `/me` 和 `/login` 校验密码）
...
7. **精确鉴权**：密码链接的 Cookie Token 校验仅限定在 `/shared-links/me` 接口，其他路由不额外校验
