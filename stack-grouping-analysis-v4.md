...
**v4 版本修正点**：
- 精确分析 `on_asset_stack_update` WebSocket 事件的 payload 传递机制
- 对比其他事件的调用方式，确认类型定义与实际实现的不一致
- 提供修正后的统一代码建议
...
**代码位置**：`server/src/services/asset.service.ts:307-330`
...
当堆栈的非主照片成员被删除时：
- `asset.stackId` 自动变为 `NULL`（数据库外键约束 `ON DELETE SET NULL`）
- 堆栈本身不受影响
...
**关键理解**：
- `ClientEventMap[T]` 定义了 `...data` 的类型
- 例如 `ClientEventMap['on_asset_delete']` 是 `[string]`，表示 1 个 string 参数
- 例如 `ClientEventMap['on_config_update']` 是 `[]`，表示 0 个参数
...
**类型表示法说明**：
- `[Type]`：元组类型，表示 1 个参数，参数类型为 `Type`
- `[]`：空元组，表示 0 个参数
- `string[]`：在 `ClientEventMap` 上下文中，表示可变数量的 `string` 参数

#### 6.1.3 所有事件调用方式对比

**代码位置**：`server/src/services/notification.service.ts`

| 事件 | 调用方式 | 传递 data | 类型定义 | 实际行为 |
|------|---------|----------|---------|---------|
| `on_notification` | `clientSend('on_notification', userId, mapNotification(item))` | 1 个参数 | `[NotificationDto]` | ✅ 一致 |
| `on_asset_hidden` | `clientSend('on_asset_hidden', userId, assetId)` | 1 个参数 | `[string]` | ✅ 一致 |
| `on_asset_trash` | `clientSend('on_asset_trash', userId, [assetId])` | 1 个参数 | `[string[]]` | ✅ 一致 |
| `on_asset_delete` | `clientSend('on_asset_delete', userId, assetId)` | 1 个参数 | `[string]` | ✅ 一致 |
| `on_asset_update` | `clientSend('on_asset_update', userId, mapAsset(...))` | 1 个参数 | `[AssetResponseDto]` | ✅ 一致 |
| `on_asset_restore` | `clientSend('on_asset_restore', userId, assetIds)` | 1 个参数 | `[string[]]` | ✅ 一致 |
| `on_session_delete` | `clientSend('on_session_delete', sessionId, sessionId)` | 1 个参数 | `[string]` | ✅ 一致 |
| `on_config_update` | 无直接调用 | 0 个参数 | `[]` | ✅ 一致 |
| **`on_asset_stack_update`** | **`clientSend('on_asset_stack_update', userId)`** | **0 个参数** | **`string[]`** | **❌ 不一致** |

#### 6.1.4 on_asset_stack_update 精确分析
...
**类型定义不一致的根因**：
- `ClientEventMap.on_asset_stack_update` 定义为 `string[]`
- 这表示 `...data` 的类型是 `string[]`，即可变数量的 string 参数
- 但实际调用时没有传递任何参数
- 正确的类型应该是 `[]`（0 个参数），与 `on_config_update` 相同
...
- 但前端代码中**没有任何地方实际订阅该事件**（搜索 `websocketEvents.on('on_asset_stack_update'` 无结果）
- 前端的堆栈更新主要通过以下方式：
  1. API 调用后的本地状态更新（如 `updateStackedAssetInTimeline`）
  2. 页面刷新时重新获取数据
  3. 依赖 `on_asset_delete` 事件间接感知

**前端本地更新示例**：`web/src/lib/utils/actions.ts`
```typescript
// 堆叠成功后更新时间线
...
| 非主照片被删除 | 数据库级联 `ON DELETE SET NULL` | ❌ 无 Stack 事件 | ❌ 无 WebSocket 通知 | `on_asset_delete` 事件 |

### 6.4 统一修正建议

#### 6.4.1 方案 A：保持无 payload（推荐，与实际行为一致）

**修改 `ClientEventMap` 类型定义**：

```typescript
// server/src/repositories/websocket.repository.ts
export interface ClientEventMap {
  // ...
  on_asset_stack_update: [];  // 改为空元组，表示 0 个参数
  // ...
}
```
...
- 与实际调用行为一致
- 前端目前没有实际订阅该事件
- 如果未来需要传递数据，可以改为 `[string[]]` 并更新调用方式

#### 6.4.2 方案 B：传递堆栈 ID 数组
...
onStackUpdate({ userId, stackId }: ArgOf<'StackUpdate'>) {
  this.websocketRepository.clientSend('on_asset_stack_update', userId, [stackId]);
}

@OnEvent({ name: 'StackDelete' })
onStackDelete({ userId, stackId }: ArgOf<'StackDelete'>) {
  this.websocketRepository.clientSend('on_asset_stack_update', userId, [stackId]);
}

@OnEvent({ name: 'StackDeleteAll' })
onStacksDelete({ userId, stackIds }: ArgOf<'StackDeleteAll'>) {
  this.websocketRepository.clientSend('on_asset_stack_update', userId, stackIds);
}
```
...
export interface ClientEventMap {
  // ...
  on_asset_stack_update: [string[]];  // 1 个参数，类型为 string[]
  // ...
}
```
...
1. **⚠️ 主照片删除不通知**：主照片被删除时，直接调用 repository 层更新，不触发 Stack 事件，前端无法感知
2. **⚠️ 非主照片删除不通知**：堆栈成员被删除时，通过数据库级联置空 `stackId`，但不会触发 `StackUpdate` 事件
3. **⚠️ WebSocket payload 不一致**：`on_asset_stack_update` 类型定义与实际实现不符
   - 类型定义：`string[]`（期望可变数量的 string 参数）
   - 实际调用：没有传递任何参数
   - 前端收到：`undefined`（完全没有参数）
4. **⚠️ 前端未订阅 WebSocket 事件**：虽然定义了事件类型，但前端没有实际订阅，依赖本地状态更新
5. **合并后主照片不可控**：合并堆栈时，新堆栈的主照片固定为 `assetIds[0]`，用户无法在合并时指定
...
4. 提供自动分组的开关和敏感度设置
5. **修复主照片删除时的事件通知**：在 `asset.service.ts` 中显式触发 `StackUpdate` 事件
6. **修复非主照片删除时的事件通知**：在资产删除逻辑中检查并触发 `StackUpdate` 事件
7. **修复 WebSocket payload**：统一类型定义与实际实现
8. 合并堆栈时允许用户选择保留哪个主照片
