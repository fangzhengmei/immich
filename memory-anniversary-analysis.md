...
1. 删除关联资产已被归档的 memory_asset 记录
2. 删除创建超过 30 天且未被保存（isSaved = false）的 memory
...
| 任务状态 | 必须通过 `asset_job_status` 关联（隐含已完成转码） | L470 |
...
在 `memory.service.ts:47-66` 的 `createOnThisDayMemories` 方法中：
...
**关键逻辑**：
- `showAt`：当天 00:00 开始显示
- `hideAt`：当天 23:59:59 结束显示
- `memoryAt`：原始照片拍摄日期（用于按时间排序）
- 按年份分组，每年同一天生成一个独立的 Memory
...
### 5.1 Memory 表结构（`memory.table.ts`）
...
### 6.1 前端数据组织（`memory-manager.svelte.ts`）
...
### 8.2 为什么每年生成独立 Memory 而不是合并？

- 用户体验：按年份分组浏览更有仪式感
- 保存粒度：可以单独保存/删除某一年的回忆
- 展示灵活：前端可以自由组织展示方式

### 8.3 为什么需要 showAt/hideAt？
...
1. **跨 Memory 资产去重**：同一张照片可能出现在不同日期±3天的回忆中
