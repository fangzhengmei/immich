...
| **资源消耗** | 极低 | 较高（双份文件 I/O + 事务开销） | 中等 |
| **排队作业** | 无 | FileDelete（清理竞态失败请求的文件） | FileDelete |
| **后续任务分发** | ❌ 不进入 | ❌ 不进入（异常在排队前抛出） | ❌ 不进入 |
| **典型场景** | 单请求正常上传 | 批量上传、并发同步 | Web 端、旧客户端 |

#### 1.4.5 为何三条路径都不会进入后续任务分发链路
...
**闭环设计的精妙之处**：
1. **性能优先**：99% 场景在第一层拦截，零资源消耗
2. **正确性兜底**：并发竞态由数据库原子性保证，绝对不会漏
3. **语义增强**：第三层处理 checksum 不同但内容相似的场景，覆盖更全面
4. **用户体验一致**：无论哪一层拦截，客户端收到的响应格式完全相同
...
| 检测层级 | 触发位置 | 检测方式 | 检测对象 | 覆盖场景 |
|---------|--------|---------|---------|---------|
| **第一级**：前置短路 | Interceptor 层 | HTTP Header checksum 预查 | 精确重复（checksum 相同） | 单请求上传、无并发场景 |
| **第二级**：约束兜底 | Service 层 | 数据库唯一索引 | 精确重复（checksum 相同） | 并发上传竞态、Header 缺失/错误 |
| **第三级**：相似度检测 | 后台作业层 | CLIP embedding 向量相似度 | 语义相似（checksum 可能不同） | 同图不同分辨率、编辑后的版本、截图 |
...
#### 1. 三条执行路径（两类短路 + 一条正常路径）

| 路径 | 触发层 | 触发条件 | 资源消耗 | 是否进入任务链 |
|-----|--------|---------|---------|---------------|
| **前置 checksum 短路** | Interceptor 层 | 客户端传 header 且 DB 已有记录 | 极低（仅 1 次 SELECT） | ❌ 不进入 |
| **竞态约束兜底** | Service 层 | 并发请求同时通过前置检查 | 较高（双份文件 I/O + 事务回滚） | ❌ 不进入 |
| **Header 缺失约束兜底** | Service 层 | 客户端未传 header 或传错 | 中等（文件 I/O + 事务回滚） | ❌ 不进入 |
| **完整作业链（正常路径）** | Service 层 | 通过所有检查 | 完整处理成本 | ✅ 进入 |
