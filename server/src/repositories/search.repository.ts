    ...
    return this.db.transaction().execute(async (trx) => {
      ...
      return await trx
        .with('cte', (qb) =>
          qb
            .selectFrom('asset_face')
            .select(['asset_face.faceClusterId', sql<number>`face_search.embedding <=> ${embedding}`.as('distance')])
            .innerJoin('asset', 'asset.id', 'asset_face.assetId')
            .innerJoin('face_search', 'face_search.faceId', 'asset_face.id')
            .leftJoin('person', 'person.faceClusterId', 'asset_face.faceClusterId')
            .where('asset.ownerId', '=', anyUuid(userIds))
            .where('asset.deletedAt', 'is', null)
            .where('asset_face.faceClusterId', 'is not', null)
        ...
        .selectFrom('cte')
        .select('cte.faceClusterId')
        .groupBy('cte.faceClusterId')
        .where('cte.distance', '<=', maxDistance)
        .execute();
    });
  }
