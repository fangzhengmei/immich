...
**v3 版本修正点**：
- 澄清主照片删除时的事件触发行为（之前版本有误）
- 澄清成员移除的实现方式（服务层更新 vs 数据库级联）
- 补齐 WebSocket 事件 payload 与前端订阅的对应关系
...
### 6.2 WebSocket 事件 Payload 分析

**服务端类型定义**：`server/src/repositories/websocket.repository.ts:30`
```typescript
export interface ClientEventMap {
  // ...
  on_asset_stack_update: string[];  // 期望接收 assetIds 数组
  // ...
}
```

**实际调用方式**：`server/src/services/notification.service.ts:189, 194, 199, 204`
```typescript
this.websocketRepository.clientSend('on_asset_stack_update', userId);
```

**⚠️ 关键发现 - Payload 不一致**：
- 类型定义期望 `string[]`（assetIds 数组）
- 但实际调用时只传了 `userId` 作为 room 参数，没有传 data 参数
- 实际上前端收到的是**空数组 `[]`**，而不是 assetIds
- 这是一个类型定义与实际实现不一致的问题
...
| 操作 | 调用路径 | 触发事件 | WebSocket 通知 |
|------|---------|---------|---------------|
| 创建堆栈 | `StackService.create()` | `StackCreate` | ✅ `on_asset_stack_update` |
| 更新主照片 | `StackService.update()` | `StackUpdate` | ✅ `on_asset_stack_update` |
| 移除成员 | `StackService.removeAsset()` | `StackUpdate` | ✅ `on_asset_stack_update` |
| 删除单个堆栈 | `StackService.delete()` | `StackDelete` | ✅ `on_asset_stack_update` |
| 批量删除堆栈 | `StackService.deleteAll()` | `StackDeleteAll` | ✅ `on_asset_stack_update` |
| 主照片被删除（自动选新主照片） | `AssetService.handleAssetDeletion()` → `stackRepository.update()` | ❌ 无 Stack 事件 | ❌ 无 WebSocket 通知 |
| 主照片被删除（堆栈只剩 1 人） | `AssetService.handleAssetDeletion()` → `stackRepository.delete()` | ❌ 无 Stack 事件 | ❌ 无 WebSocket 通知 |
| 非主照片被删除 | 数据库级联 `ON DELETE SET NULL` | ❌ 无 Stack 事件 | ❌ 无 WebSocket 通知 |
...
3. **⚠️ WebSocket payload 不一致**：`on_asset_stack_update` 类型定义期望 `string[]`，但实际发送空数组
...
7. **修复 WebSocket payload**：发送实际的 assetIds 或修改类型定义
