# 媒资入库时存储模板表达式向真实路径转换过程分析（Round 3）

> **更正说明**：本文档在 Round 2 基础上修正全量迁移触发入口的调用路径：
> - Round 1/2 错误路径：`POST /api/queue/storageTemplateMigration/start`
> - **Round 3 正确路径**：`PUT /jobs/storageTemplateMigration`（通过 jobs 接口触发 legacy queue 的 start 命令）
>
> 本文补充完整调用链的代码证据，并统一相关表述。
...
const storagePresets = [
  ...
  '{{y}}/{{#if album}}{{album-startDate-y}}/{{album}}{{else}}Other/{{MM}}{{/if}}/{{filename}}',  // 6
  '{{y}}/{{MMM}}/{{filename}}',                      // 7
  '{{y}}/{{MMMM}}/{{filename}}',                     // 8
  '{{y}}/{{MM}}/{{dd}}/{{filename}}',                // 9
...
### 5.3 全量迁移触发链（**Round 3 更正：完整调用链+证据**）
...
#### 5.3.7 全量迁移完整调用链（Round 3 最终版）

```
管理员操作
    ↓
✅ PUT /api/jobs/storageTemplateMigration    【更正：实际入口】
...
### 9.2 全量迁移触发链（**Round 3 最终版，更正 API 入口**）

```
1. 管理员通过 UI/API 启动迁移
   ↓
2. ✅ PUT /api/jobs/storageTemplateMigration        【更正：实际入口】
   Body: { "command": "start", "force": false }
   ↓
...
## 十、事实偏差更正总表（Round 1 → Round 2 → Round 3）

| 事项 | Round 1（偏差） | Round 2（修正） | Round 3（最终修正+证据） |
|------|----------------|----------------|--------------------------|
| 预设模板数量 | 22 个 | **21 个** | **21 个** |
| 预设模板来源 | 未明确 | `storage-template.service.ts:35-57` | `storage-template.service.ts:35-57` + 单元测试双重验证 |
| 全量迁移触发方式 | "模板变更自动触发" | **必须显式触发** | **必须显式触发** |
| 配置更新与迁移关系 | 未明确 | 配置更新=编译模板；迁移=独立操作 | 配置更新=编译模板；迁移=独立操作 |
| 全量迁移 API 入口 | 未提及 | `POST /api/queue/storageTemplateMigration/start` ❌ 错误 | **`PUT /api/jobs/storageTemplateMigration`** ✅ 正确<br>Body: `{"command": "start"}` |
| 控制器 | 未提及 | 未明确 | `JobController.runQueueCommandLegacy()` |
| 服务方法 | 未提及 | `queue.service:runCommandLegacy()` | `queue.service:runCommandLegacy()` → `start()` |
| 证据链 | 无 | 部分代码引用 | **完整 8 层证据链**：路由→控制器→DTO→枚举→服务分发→start方法→排队→任务执行 |

### Round 3 新增：全量迁移触发入口证据链
